...
public class search {
    ...
    */
    ...
    /*
   purpose: to search the array and find the percentages based on assessmentclass and specific neighbourhood
   parametes: ObservableList<property>data, neighbourhood name
   reutrn array of all the percentages
   "Residential","Commercial","Other Residential","FARMLAND"
   */
   static List pieChartAssessmentClassPlusNeighbourhoodSearch(List<property>data,String neighbourhood){
       List<Integer> CountAssessment = new ArrayList();
       int residentialCount =0; int commercialCount =0; int otherResidentialCount=0; int farmlandCount =0;
       for(int i=0;i<data.size();i++){
           if((data.get(i).getAssessmentClass1().equals("RESIDENTIAL")) && (data.get(i).getNeightbourhood()).equals(neighbourhood.toUpperCase())){
               residentialCount++;
           }
           else if(data.get(i).getAssessmentClass1().equals("COMMERCIAL")&& (data.get(i).getNeightbourhood()).equals(neighbourhood.toUpperCase())){
               commercialCount++;
           }
           else if(data.get(i).getAssessmentClass1().equals("OTHER RESIDENTIAL")&& (data.get(i).getNeightbourhood()).equals(neighbourhood.toUpperCase())){
               otherResidentialCount++;
           }
           else if (data.get(i).getAssessmentClass1().equals("FARMLAND")&& (data.get(i).getNeightbourhood()).equals(neighbourhood.toUpperCase())){
               farmlandCount++;
           }
       }
       CountAssessment.add(residentialCount);CountAssessment.add(commercialCount);CountAssessment.add(otherResidentialCount);CountAssessment.add(farmlandCount);
       return CountAssessment;
   }
   
   static Set findallneighbourhood(List<property>data){
        Set<String> hash_Set = new HashSet<String>();
        for(int i=0;i<data.size();i++){
            hash_Set.add(data.get(i).getNeightbourhood());
        }
        return hash_Set;
   }
